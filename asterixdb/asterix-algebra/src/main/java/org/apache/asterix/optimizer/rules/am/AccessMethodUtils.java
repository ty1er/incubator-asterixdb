...
public class AccessMethodUtils {
    ...
    public static void appendSecondaryIndexTypes(Dataset dataset, ARecordType recordType, ARecordType metaRecordType,
            Index index, boolean primaryKeysOnly, List<Object> dest, boolean requireResultOfInstantTryLock)
    ...
    public static void appendSecondaryIndexOutputVars(Dataset dataset, ARecordType recordType,
            ...
            List<LogicalVariable> dest, boolean requireResultOfInstantTryLock) throws AlgebricksException {
        int numPrimaryKeys;
        if (dataset.getDatasetType() == DatasetType.EXTERNAL) {
    ...
    public static List<LogicalVariable> getKeyVarsFromSecondaryUnnestMap(Dataset dataset, ARecordType recordType,
            ARecordType metaRecordType, ILogicalOperator unnestMapOp, Index index, int keyType,
            boolean outputPrimaryKeysOnlyFromSIdxSearch) throws AlgebricksException {
        int numPrimaryKeys;
        int numSecondaryKeys = KeyFieldTypeUtil.getNumSecondaryKeys(index, recordType, metaRecordType);
        if (dataset.getDatasetType() == DatasetType.EXTERNAL) {
            numPrimaryKeys = IndexingConstants
        ...
        List<LogicalVariable> sourceVars = ((AbstractUnnestMapOperator) unnestMapOp).getVariables();
     ...
     * @throws AlgebricksException
     */
    public static Pair<ILogicalExpression, ILogicalExpression> createSearchKeyExpr(boolean isEnforcedIndex, IOptimizableFuncExpr optFuncExpr,
            IAType indexedFieldType, OptimizableOperatorSubTree  probeSubTree)
            throws AlgebricksException {
        if (probeSubTree == null) {
            ...
            if (optFuncExpr.getNumConstantExpr() == 0) {
                ...
                return new Pair<>(new VariableReferenceExpression(optFuncExpr.getLogicalVar(1)), null);
            }
            ...
            TypeCastingMathFunctionType mathFunctionTypeForNumericTypeCasting = TypeCastingMathFunctionType.NONE;
            ...
            if (constantValueTag != indexedFieldTypeTag && constantValue != null) {
                        ...
                        switch (indexedFieldTypeTag) {
                            ...
                            case BIGINT:
                                ...
                                switch (cKind) {
                                    ...
                                    case GE:
                                        // round-up
                                        mathFunctionTypeForNumericTypeCasting = TypeCastingMathFunctionType.CEIL;
                                        break;
                                    case LE:
                                    case GT:
                                        // round-down
                                        ...
                                        break;
                                    case EQ:
                                        // equality case - both needs to be applied.
                ...
                switch (mathFunctionTypeForNumericTypeCasting) {
                    case CEIL_FLOOR:
                        try {
                            replacedConstantValue = ATypeHierarchy.getAsterixConstantValueFromNumericTypeObject(
                                    ...
                                    TypeCastingMathFunctionType.FLOOR);
                            replacedConstantValueForEQCase = ATypeHierarchy
                                    .getAsterixConstantValueFromNumericTypeObject(constantValue.getObject(),
                                            indexedFieldTypeTag, isEnforcedIndex, TypeCastingMathFunctionType.CEIL);
                        } catch (HyracksDataException e) {
                            throw new CompilationException(ErrorCode.ERROR_OCCURRED_BETWEEN_TWO_TYPES_CONVERSION, e,
                                    constantValueTag, indexedFieldTypeTag);
                        }
                        break;
                    case CEIL:
                    case FLOOR:
            ...
            if (typeCastingApplied) {
                if (replacedConstantValueForEQCase == null) {
                    return new Pair<>(new ConstantExpression(replacedConstantValue), null);
                } else {
                    // For equality case
                ...
                return new Pair<>(optFuncExpr.getConstantExpr(0), null);
            }
        } else {
            ...
            if (ATypeHierarchy.belongsToDomain(indexedFieldTypeTag, ATypeHierarchy.Domain.NUMERIC)) {
                ...
                if (probeTypeTypeTag != indexedFieldTypeTag) {
                    ...
                    return new Pair<>(castFunc, null);
                }
            }
            return new Pair<>(probeExpr, null);
        }
    }
    ...
    public static boolean finalizeJoinPlanTransformation(List<Mutable<ILogicalOperator>> afterJoinRefs,
            ...
            Mutable<ILogicalExpression> conditionRef, Dataset dataset)
            throws AlgebricksException {
        if (isLeftOuterJoin && hasGroupBy) {
            ...
            indexSearchOp = AccessMethodUtils.resetVariableMappingInUnionOpInIndexOnlyPlan(lojMissingVarExist,
        ...
        indexSubTree.getDataSourceRef().setValue(indexSearchOp);
        if (conditionRef.getValue() != null) {
            ...
            if (isIndexOnlyPlan && dataset.getDatasetType() == DatasetType.INTERNAL) {
                ...
                joinRef.setValue(indexSearchOp);
            } else {
    ...
    public static ILogicalOperator createSecondaryIndexUnnestMap(Dataset dataset, ARecordType recordType,
        ...
        appendSecondaryIndexOutputVars(dataset, recordType, metaRecordType, index, outputPrimaryKeysOnly, context,
                secondaryIndexUnnestVars, generateInstantTrylockResultFromIndexSearch);
        ...
        if (retainNull) {
            if (retainInput) {
                ...
                secondaryIndexLeftOuterUnnestOp
                        .setGenerateConditionalSplitVar(generateInstantTrylockResultFromIndexSearch);
            ...
            UnnestMapOperator secondaryIndexUnnestOp = new UnnestMapOperator(secondaryIndexUnnestVars,
                    ...
                    retainInput);
            secondaryIndexUnnestOp.setGenerateConditionalSplitVar(generateInstantTrylockResultFromIndexSearch);
    ...
    private static AbstractUnnestMapOperator createRestOfNonIndexOnlySearchPlan(Dataset dataset,
    ...
    public static ILogicalOperator createRestOfIndexSearchPlan(List<Mutable<ILogicalOperator>> afterTopOpRefs,
            ...
            OptimizableOperatorSubTree subTree, boolean outputPrimaryKeysOnlyFromSIdxSearch,
        ...
        List<LogicalVariable> pkVarsFromSIdxUnnestMapOp = AccessMethodUtils.getKeyVarsFromSecondaryUnnestMap(dataset,
                recordType, metaRecordType, inputOp, secondaryIndex, 0, outputPrimaryKeysOnlyFromSIdxSearch);
        ...
        List<LogicalVariable> skVarsFromSIdxUnnestMap = AccessMethodUtils.getKeyVarsFromSecondaryUnnestMap(dataset,
                recordType, metaRecordType, inputOp, secondaryIndex, 1, outputPrimaryKeysOnlyFromSIdxSearch);
        ...
        if (!isIndexOnlyPlan) {
            return createRestOfNonIndexOnlySearchPlan(dataset, inputOp, context, sortPrimaryKeys, retainInput,
        ...
        ProjectOperator leftProjectBeforeUnionAllOp;
        ProjectOperator rightProjectBeforeUnionAllOp;
        ...
        if (assignsBeforeTopOpRef != null) {
            ...
            if (!constAssignVars.isEmpty()) {
                if (!constantAssignVarUsedInTopOp) {
                    // Places this assign at the bottom of the operators after SELECT or JOIN operator.
                    constAssignOp = new AssignOperator(constAssignVars, constAssignExprs);
                    constAssignOp.getInputs().add(new MutableObject<ILogicalOperator>(topOpRef.getValue()));
                    constAssignOp.setExecutionMode(ExecutionMode.PARTITIONED);
                    context.computeAndSetTypeEnvironmentForOperator(constAssignOp);
                    ILogicalOperator currentOpAfterTopOp = afterTopOpRefs.get(afterTopOpRefs.size() - 1).getValue();
        ...
        if (skVarAssignOpInRightPath != null) {
            skVarAssignOpInRightPath.getInputs().clear();
        ...
        // Adds a PROJECT operator in both paths before UNIONALL operator to unify the variables from each branch.
        ...
        // Creates the variable list for the PROJECT operators in both path.
        for (Triple<LogicalVariable, LogicalVariable, LogicalVariable> uVars : unionVarMap) {
            // Variables from the left branch
            leftVars.add(uVars.first);
            // Variables from the right branch
            rightVars.add(uVars.second);
        }

        // PROJECT operator for the left side
        leftProjectBeforeUnionAllOp = new ProjectOperator(leftVars);
        leftProjectBeforeUnionAllOp.getInputs().add(new MutableObject<ILogicalOperator>(newSelectOpInLeftPath));
        ...
        // UNIONALL operator that combines both PROJECT operators.
    ...
    public static UnnestMapOperator createExternalDataLookupUnnestMap(AbstractDataSourceOperator dataSourceOp,
        ...
        List<LogicalVariable> primaryKeyVars = AccessMethodUtils.getKeyVarsFromSecondaryUnnestMap(dataset, recordType,
                metaRecordType, inputOp, secondaryIndex, 0, outputPrimaryKeysOnlyFromSIdxSearch);
    ...
    public static void indexOnlyPlanCheck(List<Mutable<ILogicalOperator>> afterTopRefs,
        ...
        // Need to check whether variables from the given select (join) operator only contain SK and/or PK condition.
        List<IOptimizableFuncExpr> matchedFuncExprs = analysisCtx.getMatchedFuncExprs();

        // Fetches the field names of the primary index and chosen index.
        ...
        if (dataset.getDatasetType() == DatasetType.EXTERNAL || chosenIndex.isPrimaryIndex()
            ...
            checkVarUsageAfterSelectOp(afterTopRefs, liveVarsAfterSelJoinOp, dataScanPKVars, chosenIndexVars,
                    chosenIndex, indexSubTree, chosenIndexFieldNames, dataScanRecordVars, context, constantVars,
                    indexOnlyPlanInfo);
        ...
        if (isInvertedIndex(chosenIndex)) {
            isIndexOnlyPlan = false;
        }
                                ...
                                if (tmpValFieldName.contains(fieldName)) {
                                    try {
                                        ...
                                        throw CompilationException
                                                .create(ErrorCode.CANNOT_GET_FIELD_TYPE_FOR_FIELD_NAME, e, fieldName);
                                    }
                                    break;
                                }
                            }
    ...
    public static ILogicalOperator findDataSourceFromIndexUtilizationPlan(ILogicalOperator topOp) {
        ...
        switch (topOp.getOperatorTag()) {
            ...
            case UNIONALL:
                ...
                // If the operator before the UNIONALL is PROJECT, then it's an index-only plan.
                if (dataSourceOp.getOperatorTag() == LogicalOperatorTag.PROJECT) {
                    break;
                }
                return null;
            default:
        ...
        // The order of operators: 7 union <- 6 project (current) <- 5 select <- 4 assign?
        // <- 3 unnest-map (PIdx) <- 2 split <- 1 unnest-map (SIdx) <- ...
        // We do this to skip the primary index-search since we are looking for a secondary index-search here.
        do {
            dataSourceOp = (ILogicalOperator) dataSourceOp.getInputs().get(0).getValue();
        } while (dataSourceOp.getOperatorTag() != LogicalOperatorTag.SPLIT);
     ...
     * in case of group-by Missing expression in Left-Outer-Join (LOJ). Also adjusts
     * the PROJECT operators in both paths of the index-only plan before UNIONALL operator.
     *
     * @throws AlgebricksException
     */
    public static ILogicalOperator resetVariableMappingInUnionOpInIndexOnlyPlan(boolean LOJVarExist,
            List<LogicalVariable> LOJMissingVariables, ILogicalOperator unionOp,
        ...
        if (unionOp.getOperatorTag() != LogicalOperatorTag.UNIONALL) {
            return unionOp;
        }
        ...
        List<Triple<LogicalVariable, LogicalVariable, LogicalVariable>> varMap =
                ((UnionAllOperator) unionOp).getVariableMappings();
        ...
        // When changes in the variable mapping are happened, we need to adjust
        // Project operator before UNIONALL operators.
        if (LOJVarExist && LOJVarCanBeDeleted) {
            UnionAllOperator unionAllOp = new UnionAllOperator(varMap);
            ProjectOperator leftProjectBeforeUnionAllOp;
            ProjectOperator rightProjectBeforeUnionAllOp;
            ILogicalOperator leftOp = unionOp.getInputs().get(0).getValue();
            ILogicalOperator rightOp = unionOp.getInputs().get(1).getValue();

            // Changes the Project operator before UNIONALL operator, too.
            int projectOpCount = 0;
            for (int i = 0; i < unionOp.getInputs().size(); i++) {
                ILogicalOperator op = unionOp.getInputs().get(i).getValue();
                if (op.getOperatorTag() == LogicalOperatorTag.PROJECT) {
                    projectOpCount++;
                }
            }

            // Project operators are already there. We need to replace them
            if (projectOpCount == unionOp.getInputs().size()) {
                // Replace the current PROJECT operators on the both side
                // before UNION operator to unify the variables from each branch.
                List<LogicalVariable> leftVars = new ArrayList<>();
                List<LogicalVariable> rightVars = new ArrayList<>();
