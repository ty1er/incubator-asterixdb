...
public class IntroduceJoinAccessMethodRule extends AbstractIntroduceAccessMethodRule {
    ...
    protected boolean checkAndApplyJoinTransformation(Mutable<ILogicalOperator> opRef, IOptimizationContext context)
        ...
        if (isThisOpInnerJoin) {
            ...
            // Since the child of the current GROUP operator is left-outer-join,
            // we need to add the current operator to the operator list that contains
            // all operators after the join operator.
            afterJoinRefs.add(opRef);
        } else {
            // This is not a JOIN operator. So, we only gather necessary information
            // that is going to be used for checking index-only plan.
            afterJoinRefs.add(opRef);
        }

        // Recursively checks the plan and try to optimize it. This first checks the children of the given operator
        ...
        if (isThisOpInnerJoin || isThisOpLeftOuterJoin) {
            ...
            if (continueCheck && checkRightSubTreeMetadata) {
                ...
                if (continueCheck) {
                    ...
                    // If the chosen index is the primary index - also maps the field name to
                    // the variable(s) of this index-search. This is necessary for the index-only-plan check.
                    ArrayList<LogicalVariable> pkVars = new ArrayList<>();
                    if (chosenIndex.second.isPrimaryIndex()) {
                        rightSubTree.getPrimaryKeyVars(null, pkVars);
                        List<List<String>> chosenIndexFieldNames = chosenIndex.second.getKeyFieldNames();
                        for (int i = 0; i < pkVars.size(); i++) {
                            if (!rightSubTree.getVarsTofieldNameMap().containsKey(pkVars.get(i))) {
                                rightSubTree.getVarsTofieldNameMap().put(pkVars.get(i), chosenIndexFieldNames.get(i));
                            }
                        }
                    }
        ...
        // Cleans the path after JOIN operator by removing the current operator.
