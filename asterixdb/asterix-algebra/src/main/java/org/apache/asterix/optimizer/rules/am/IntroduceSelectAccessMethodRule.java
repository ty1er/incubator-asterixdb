 ...
 * The use of an index is expressed as an unnest-map over an index-search function which will be
 ...
 * NEW:
 * the left path- union <-- select <-- assign+ <-- unnest-map(pidx) <-- split <-- unnest-map(sidx) <-- assign
 * the right path union <-- ....................................... <-- split
 ...
 * If it succeeds, the given <SK, PK> pair trustworthy so that we can return this as a valid output.
 * This tuple goes to the right path of UNION operator since we don't need to traverse the primary index.
 ...
 * goes to the left path of UNION operator to traverse the primary index.
 * In the left path, we fetch the record using the given PK and fetch SK field and does SELECT verification.
 * 2) UNION operators combines tuples from the left path and the right path and the rest of the plan continues.
 * In an index-only plan, sort before the primary index-search is not required since we assume that
 * the changes that a tuple (<SK, PK> pair) goes into the left path are low.
 * If a query plan is not an index-only plan, we call this plan as non-index only plan.
 * In this case, the original plan will be transformed into this:
        ...
        if (isSelectOp) {
            ...
            if (continueCheck) {
                ...
                if (chosenIndexes.size() > 1) {
                    ...
                    // Since only a single (secondary) index is chosen, checks whether this plan is an index-only plan.
                    ...
                    // If the chosen index is the primary index, adds variable:fieldname to subTree.fieldNames.
                    ArrayList<LogicalVariable> pkVars = new ArrayList<>();
                    if (chosenIndexes.get(0).second.isPrimaryIndex()) {
                        subTree.getPrimaryKeyVars(null, pkVars);
                        List<List<String>> chosenIndexFieldNames = chosenIndexes.get(0).second.getKeyFieldNames();
                        for (int i = 0; i < pkVars.size(); i++) {
                            subTree.getVarsTofieldNameMap().put(pkVars.get(i), chosenIndexFieldNames.get(i));
                        }
                    }
