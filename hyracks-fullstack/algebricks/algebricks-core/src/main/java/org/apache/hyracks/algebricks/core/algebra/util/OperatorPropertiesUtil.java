...
public class OperatorPropertiesUtil {
    ...
    public static void typeOpRec(Mutable<ILogicalOperator> r, IOptimizationContext context) throws AlgebricksException {
        typeOpRec((AbstractLogicalOperator) r.getValue(), context);
    }
     ...
     * @throws AlgebricksException
     */
    public static void typeOpRec(AbstractLogicalOperator lOp, IOptimizationContext context) throws AlgebricksException {
        for (Mutable<ILogicalOperator> i : lOp.getInputs()) {
            typeOpRec(i, context);
        }
        if (lOp.hasNestedPlans()) {
            for (ILogicalPlan p : ((AbstractOperatorWithNestedPlans) lOp).getNestedPlans()) {
                typePlan(p, context);
            }
        }
        context.computeAndSetTypeEnvironmentForOperator(lOp);
    }
