...
public class AqlMetadataProvider implements IMetadataProvider<AqlSourceId, String> {
    ...
    public Pair<IOperatorDescriptor, AlgebricksPartitionConstraint> getInsertOrDeleteRuntime(IndexOperation indexOp,
        ...
        // Move key fields to front.
        int permutationSize = numKeys + numFilterFields;
        if (indexOp == IndexOperation.INSERT) {
            // reserves a space for the inserted record
            permutationSize += 1;
            numPayloadFields = 1;
        }
        int[] fieldPermutation = new int[permutationSize];
        int[] bloomFilterKeyFields = new int[numKeys];
        int i = 0;
        for (LogicalVariable varKey : keys) {
            int idx = propagatedSchema.findVariable(varKey);
        ...
        if (numFilterFields > 0) {
            int idx = propagatedSchema.findVariable(additionalNonKeyFields.get(0));
            fieldPermutation[numKeys + numPayloadFields] = idx;
        }
    ...
    public Pair<IOperatorDescriptor, AlgebricksPartitionConstraint> getDeleteRuntime(
            IDataSource<AqlSourceId> dataSource, IOperatorSchema propagatedSchema, IVariableTypeEnvironment typeEnv,
            List<LogicalVariable> keys, List<LogicalVariable> additionalNonKeyFields, RecordDescriptor recordDesc,
            JobGenContext context, JobSpecification spec) throws AlgebricksException {
        return getInsertOrDeleteRuntime(IndexOperation.DELETE, dataSource, propagatedSchema, typeEnv, keys, null,
